...
jQuery(function () {
	var originTest = {
		'test1': {
			...
			'answers': [{ 'correct': 'Не вмикнули комп\'ютера' }, { 'wrong': 'Вчора щось не те з\'їли' }, { 'wrong': 'Ананаси і єноти!!!' }]
		},
		'test2': {
			'question': 'Вмикнули бравзера, а всі сайти якось\
					дивно відображаються. Як виправити недолік?',
			'answers': [{ 'correct': 'Під\'єднати інтернет' }, { 'correct': 'Не користуватися бравзерами Internet Explorer' }, { 'wrong': 'Постукати по монітору' }]
		},
		'test3': {
			'question': 'На сайтах все шевелиться, анімується,\
					змінює колір — це...',
			'answers': [{ 'correct': 'javaScript' }, { 'wrong': 'хтось трясе твого монітора' }, { 'wrong': 'класно!' }]
		}
	};
	...
	var writeTest = function writeTest() {
		return $('ol').html(tmpl('madtest', { 'test': getTest() }));
	};

	var checkResults = function checkResults(event) {
		var val = 0;
		var diagnos = void 0;
