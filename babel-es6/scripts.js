...
jQuery(function () {
	...
	let test = JSON.parse(localStorage['madTest']);


	let getTest = () => {
		for (let key in test) {
			test[key]['answers'] = test[key]['answers'].sort(() => Math.random() - 0.5);
		}
		return test;
	};


	let writeTest = () => $('ol').html(tmpl( 'madtest', {'test': getTest()} ));


	let checkResults = (event) => {
		let val = 0;
		let diagnos;
		...
		$('.modal-window').css('margin-top', () => $('.modal').height() / 2 - $('.modal-window').height());
